...
/* For custom template */
#define TEMPLATE_MANUFACTURER_SPECIFIC_COMPANY_ID 0x061A
...
/* Format of the Manufacturer Specific Data (MSD) for custome template.
 */
struct TemplateSensorAdEvent {
	uint16_t companyId;
	uint8_t data1;
	uint16_t data2;
	uint8_t data3;
...
/* Format of thef response payload for 1M PHY.
...
typedef struct TemplateSensorAdEvent  TemplateSensorAdEvent_t;
...
/*
 * This is the format for custom template.
 */
#define TEMPLATE_MSD_AD_FIELD_LENGTH 0x07
#define TEMPLATE_MSD_AD_PAYLOAD_LENGTH (TEMPLATE_MSD_AD_FIELD_LENGTH - 1)
// BUILD_ASSERT(sizeof(TemplateSensorAdEvent_t) == TEMPLATE_MSD_AD_PAYLOAD_LENGTH,
// 	     "Template Advertisement data size mismatch (check packing)");
...
extern const uint8_t TEMPLATE_AD_HEADER[2];
